...
def operator_summary():
    ...
    Filter correction — a period-WIDE, not per-row, decision between two
    modes:
      - "activity": at least one active product has genuine finalized
        ...
        straight from daily_figure_view(), never recomputed). Opening
        Stock/Closing Stock alone never qualifies a product, and a
        draft-only, voided, reset-neutralized, or superseded record never
        counts either — every count here is the same finalized-only
        query every other Daily Figures surface already uses.
    ...
    figures (there is no ratio to convert against) — it is guarded
