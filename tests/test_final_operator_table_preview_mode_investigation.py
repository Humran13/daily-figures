...
The endpoint returns one of two period-wide (never per-row) modes:
  ...
  - "activity": at least one product has genuine finalized activity —
    ...
    real authoritative values. Untouched products (including
    Opening/Closing-only ones) are hidden entirely — see
    test_final_operator_daily_figures_table_investigation.py for the
    dedicated absence/filtering tests.
"""
import pytest
